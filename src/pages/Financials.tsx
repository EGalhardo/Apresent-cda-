          ...
          <div className="h-60">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={revenueProjection} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="projected" name="Receita" stroke="#3b82f6" fill="url(#gProj)" strokeWidth={2} dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
            ...
            <div className="h-44">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={scenarioData} margin={{ top: 0, right: 5, left: 0, bottom: 0 }}>
                  ...
                  <Bar dataKey="capital" name="Capital Solicitado" fill="#6366f1" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="revenue" name="Receita Projetada" fill="#10b981" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
                ...
                <div key={s.scenario} className={`p-3 rounded-xl border ${i === 1 ? 'border-blue-200 bg-blue-50' : 'border-gray-100 bg-gray-50'}`}>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="text-xs text-gray-500">Capital: <span className="font-bold text-gray-800">{formatAOA(s.capital, true)}</span></p>
                      <p className="text-xs text-gray-500">Receita: <span className="font-bold text-emerald-600">{formatAOA(s.revenue, true)}</span></p>
                    </div>
                  </div>
          ...
          <Card>
            ...
            <div className="mt-3 space-y-3">
                ...
                return (
                  <div key={item.name}>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span className="font-bold text-gray-900">{formatAOA(item.value, true)} <span className="text-gray-400 font-normal text-xs">({pct.toFixed(1)}%)</span></span>
                    </div>
              ...
              <div className="pt-2 border-t border-gray-100 flex justify-between text-sm font-bold">
                ...
                <span className="text-blue-600">{formatAOA(34590000, true)}</span>
              </div>
            </div>
          </Card>
        </div>
          ...
          {trancheData.map((t, i) => (
            <div key={t.tranche} className="relative">
              ...
              <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100 hover:border-blue-200 transition-colors">
                ...
                <p className="text-xl font-bold mb-2" style={{ color: t.color }}>{formatAOA(t.amount, true)}</p>
      ...
      <Card>
        <div className="flex items-center justify-between mb-4">
          ...
          <Badge variant={cashFlow[cashFlow.length - 1].cumulative >= 0 ? 'green' : 'red'}>
            {formatAOA(cashFlow[cashFlow.length - 1].cumulative, true)} acumulado
          </Badge>
        </div>
        <div className="h-56">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={cashFlow} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="cumulative" name="Fluxo Acumulado" stroke="#10b981" fill="url(#gCash)" strokeWidth={2.5} dot={false} />
              <Area type="monotone" dataKey="net" name="Resultado Mensal" stroke="#6366f1" fill="none" strokeWidth={1.5} strokeDasharray="4 4" dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </Card>
